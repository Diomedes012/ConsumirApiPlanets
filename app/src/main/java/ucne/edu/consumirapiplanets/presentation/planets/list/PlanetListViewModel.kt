...
import ucne.edu.consumirapiplanets.domain.usecase.GetPlanetsUseCase
...
class PlanetListViewModel @Inject constructor(
    ...
    fun onEvent(event: PlanetListEvent) {
        when (event) {
            is PlanetListEvent.UpdateName -> _state.update {
    ...
    private fun loadPlanets() {
        viewModelScope.launch {
            ...
            getPlanetsUseCase(name = current.filterName).collect { result ->
                when (result) {
